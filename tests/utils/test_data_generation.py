...
class TestDataGeneration(unittest.TestCase):
    ...
    def test_generate_attention_data_custom(self) -> None:
        ...
        queries, keys, values = generate_attention_data(batch_size, seq_length, d_k, d_v)
    ...
    def test_generate_attention_data_distribution(self) -> None:
        queries, keys, values = generate_attention_data(batch_size=100, seq_length=100, d_k=10, d_v=10)
        ...
        def check_normal_distribution(data: np.ndarray, name: str) -> None:
            ...
            self.assertGreater(p_val, 0.05, f"{name} mean significantly different from 0")

            self.assertAlmostEqual(np.std(data), 1.0, delta=0.1, msg=f"{name} std dev not close to 1")
